/**
 * Fetch historical OHLC data from Coinglass API and compute indicators
 * Usage: node scripts/fetch_coinglass.js <symbol> <timeframe> [outputDir]
 * Example: node scripts/fetch_coinglass.js BTC 1h data
 */
...
async function fetchCoinglassOHLC(symbol, timeframe, limit = 500) {
  const apiKey = process.env.COINGLASS_API_KEY;
  if (!apiKey) {
    throw new Error('COINGLASS_API_KEY not set in .env');
  }
  
  const coinglassSymbol = COINGLASS_SYMBOLS[symbol.toUpperCase()];
  if (!coinglassSymbol) {
    throw new Error(`Unsupported symbol: ${symbol}. Available: ${Object.keys(COINGLASS_SYMBOLS).join(', ')}`);
  }
  ...
  if (!interval) {
    throw new Error(`Unsupported timeframe: ${timeframe}. Available: ${Object.keys(intervalMap).join(', ')}`);
  }
  
  // Use spot endpoint (more stable, symbol format: BTCUSDT)
  const url = `https://open-api.coinglass.com/api/v1/spot/kline?symbol=${coinglassSymbol}USDT&interval=${interval}&limit=${limit}`;
  ...
  if (!res.ok) {
    ...
    throw new Error(`Coinglass API error: ${res.status} ${err}`);
  }
  
  const data = await res.json();
  
  if (data.code !== 200 || !data.data) {
    throw new Error(`Coinglass API error: code=${data.code}, msg=${data.msg || 'no data'}`);
  }
  ...
  for (const candle of data.data) {
    if (candle.length >= 6) {
      candles.push({
        timestamp: new Date(candle[0]), // milliseconds
...
async function fetchAndProcess(symbol, timeframe, outputDir = 'data') {
  try {
    console.log(`\n📈 Fetching ${symbol} ${timeframe} from Coinglass...`);
    const candles = await fetchCoinglassOHLC(symbol, timeframe, 2000);
...
if (!symbol || !timeframe) {
  console.error('Usage: node scripts/fetch_coinglass.js <symbol> <timeframe> [outputDir]');
  console.error('Example: node scripts/fetch_coinglass.js BTC 1h data');
  process.exit(1);
}
