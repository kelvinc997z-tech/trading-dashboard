        ...
        <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
          {signals.map((signal) => (
            <tr key={signal.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
              ...
              <td className="px-6 py-4 whitespace-nowrap text-gray-500 dark:text-gray-400 flex items-center gap-1">
                <Clock className="w-4 h-4" />
                {signal.time}
              </td>
              <td className="px-6 py-4 whitespace-nowrap">
                <span
                  className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${statusColors[signal.status]}`}
                >
