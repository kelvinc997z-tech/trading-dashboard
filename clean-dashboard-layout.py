#!/usr/bin/env python3
"""
Clean dashboard layout replacement
"""
...
# Find the return statement (first occurrence after the component function)
# Look for "return ("
...
# New return block (everything from "return (" to the closing ");" at the end)
new_return = '''return (
...
# Find the end of the return block (the last closing ); before the final })
# We want to replace from start_idx to just before the closing "};" or "}"
# Simpler: find the last "}" before "export default"
# Actually, we'll replace from start_idx to the end of the file, keeping everything after the closing ); 

# Find where the component ends (before any following code)
# Look for the pattern: "}\n\nexport default" or similar
end_match = re.search(r'\n\s*\}\s*$', content)
if not end_match:
    # Try alternative: find "export default" or "};" after return
    end_match = re.search(r'export default DashboardShell', content)
    if end_match:
        # Everything before "export default" is the component body
        component_end = end_match.start()
    else:
...
# Replace content from return start to component_end
...
print('✅ Dashboard layout refactored successfully')
print('📐 Features:')
print('  - Consistent max-w-7xl container')
print('  - Fixed header height h-16')
print('  - Clean stats grid (2-col mobile, 4-col desktop)')
print('  - Uniform card styling (rounded-xl, border)')
print('  - Simplified section spacing')
print('  - Compact trades table')
print('  - Better responsive breakpoints')
